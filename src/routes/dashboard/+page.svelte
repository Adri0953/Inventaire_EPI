<script lang="ts">
	...
	const tauxSansEquipement = $derived(
		stats.totalDrivers > 0 ? Math.round((stats.driversWithoutEquipment / stats.totalDrivers) * 100) : 0
	);
</script>

<div class="w-full max-w-7xl mx-auto p-4 lg:p-10 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">

	<!-- ── ALERTES PRIORITAIRES ──────────────── -->
	<section class="space-y-6">
		<div class="flex items-center justify-between p-6 rounded-3xl shadow-xl border-b-4 transition-colors duration-500
			{totalAlerts > 0
				? 'bg-red-600 border-red-400'
				: 'bg-emerald-800 border-emerald-600'}">
			<div class="flex items-center gap-4">
				<div class="p-3 backdrop-blur-md rounded-2xl border border-white/20 transition-colors duration-500
			...
			{#if totalAlerts > 0}
				<span class="px-5 py-2 bg-white/20 text-white text-sm font-black rounded-2xl shadow-lg animate-pulse border-2 border-white/30">
					{totalAlerts} À TRAITER
				</span>
			{:else}
				<span class="px-5 py-2 bg-white/20 text-white text-sm font-black rounded-2xl shadow-lg border-2 border-white/30">
					Tout est en règle
				</span>
			{/if}
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
			...
			<!-- Expirations -->
			<div class="group relative bg-white rounded-4xl shadow-xl hover:shadow-2xl transition-all duration-500 border-2 border-red-100 overflow-hidden flex flex-col h-160">
				<div class="p-8 bg-linear-to-br from-red-600 to-red-500 relative overflow-hidden">
					...
					<div class="relative z-10 flex items-center justify-between">
						...
						<div class="bg-white/20 backdrop-blur-md rounded-2xl px-3 py-1 border border-white/30">
							<span class="text-sm font-black text-white tabular-nums">{alerts.expiring.length}</span>
						</div>
					</div>
				</div>
				<div class="p-4 overflow-y-auto flex-1 space-y-3 bg-slate-50/30">
					{#if alerts.expiring.length > 0}
						{#each alerts.expiring as epi (epi.id_epi)}
							...
							<div class="flex items-center justify-between p-4 rounded-2xl bg-white border border-red-50 hover:border-red-200 transition-all shadow-sm">
								<div class="min-w-0">
									...
									<p class="text-xs text-slate-400 flex items-center gap-1 mt-1 font-medium">
										<Users class="w-3 h-3" /> {epi.chauffeur_nom || 'Non attribué'}
									</p>
								</div>
								<div class="text-right shrink-0">
									<p class="text-[10px] font-bold text-slate-400 tabular-nums uppercase tracking-tighter">{formatDate(epi.date_expiration)}</p>
									{#if days < 0}
										<div class="mt-1 px-2.5 py-0.5 bg-red-600 text-[9px] font-black text-white rounded-full shadow-lg shadow-red-200">EXPIRÉ</div>
									{:else}
										<span class="text-sm font-black {days <= 7 ? 'text-red-500' : 'text-slate-600'}">J-{days}</span>
									{/if}
								</div>
							</div>
						{/each}
			...
			<!-- Maintenance -->
			<div class="group relative bg-white rounded-4xl shadow-xl hover:shadow-2xl transition-all duration-500 border-2 border-orange-100 overflow-hidden flex flex-col h-160">
				<div class="p-8 bg-linear-to-br from-orange-500 to-orange-400 relative overflow-hidden">
					...
					<div class="relative z-10 flex items-center justify-between">
						...
						<div class="bg-white/20 backdrop-blur-md rounded-2xl px-3 py-1 border border-white/30">
							<span class="text-sm font-black text-white tabular-nums">{alerts.controls.length}</span>
						</div>
					</div>
				</div>
				<div class="p-4 overflow-y-auto flex-1 space-y-3 bg-slate-50/30">
	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
		...
		<!-- Inventaire total -->
		<div class="lg:col-span-2 relative rounded-3xl p-6 overflow-hidden shadow-2xl border-2 border-blue-900 flex flex-col justify-between"
			 style="background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)">
			<Package class="absolute -right-4 -bottom-4 w-32 h-32 text-white/5 opacity-10" />
			<div class="relative z-10">
				<p class="text-blue-300 font-black uppercase tracking-widest text-[10px]">Inventaire Total</p>
				<h3 class="text-5xl font-black text-white mt-1 tabular-nums leading-none tracking-tighter">{stats.total}</h3>
			</div>
			<div class="relative z-10 flex gap-2 mt-4 flex-wrap">
				<div class="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-xl border border-white/10 backdrop-blur-sm">
		...
		<!-- Taux d'attribution -->
		<div class="relative rounded-3xl p-6 shadow-2xl flex flex-col justify-between overflow-hidden border-2 border-indigo-900"
			...
			<div class="relative z-10">
				...
				<div class="flex items-end gap-1 mt-1">
					<span class="text-5xl font-black text-white tabular-nums leading-none tracking-tighter">{tauxAttribution}</span>
					<span class="text-2xl font-black text-indigo-300 mb-0.5">%</span>
				</div>
				<p class="text-indigo-200 text-xs font-bold uppercase tracking-widest mt-1">Taux d'attribution</p>
			</div>
			<div class="relative z-10 mt-4">
				<div class="h-2 bg-white/10 rounded-full overflow-hidden border border-white/10">
					<div class="h-full rounded-full bg-white/70 transition-all duration-700 shadow-[0_0_8px_rgba(255,255,255,0.3)]"
						 style="width: {tauxAttribution}%"></div>
				</div>
		...
		<!-- Chauffeurs sans EPI -->
		<div class="relative rounded-3xl p-6 shadow-2xl flex flex-col justify-between overflow-hidden border-2 transition-colors duration-500
			...
			<div class="relative z-10">
				<p class="font-black uppercase tracking-widest text-[10px] transition-colors duration-500
					{stats.driversWithoutEquipment > 0 ? 'text-orange-200' : 'text-emerald-200'}">Chauffeurs</p>
				<div class="flex items-end gap-1 mt-1">
					<span class="text-5xl font-black text-white tabular-nums leading-none tracking-tighter">{stats.driversWithoutEquipment}</span>
				</div>
				<p class="text-xs font-bold uppercase tracking-widest mt-1 text-white/70">Sans équipement</p>
			</div>
			<div class="relative z-10 mt-4">
				<div class="h-2 bg-white/10 rounded-full overflow-hidden border border-white/10">
					<div class="h-full rounded-full bg-white/70 transition-all duration-700 shadow-[0_0_8px_rgba(255,255,255,0.3)]"
						 style="width: {tauxSansEquipement}%"></div>
				</div>
				<div class="flex justify-between mt-1.5">
					<span class="text-[10px] font-bold text-white/60">
						{stats.driversWithoutEquipment > 0 ? 'À équiper' : 'Tous équipés ✓'}
					</span>
					<span class="text-[10px] font-bold text-white/60">{stats.totalDrivers} chauffeur{stats.totalDrivers > 1 ? 's' : ''}</span>
				</div>
			</div>
		</div>
			...
			<div class="divide-y divide-slate-50">
				{#each activity.assignments as item (item.id_attribution)}
					<div class="px-8 py-5 flex items-center gap-4 hover:bg-slate-50/50 transition-colors">
						<div class="w-10 h-10 rounded-2xl bg-blue-100 flex items-center justify-center font-black text-blue-700 text-sm">
						...
						<span class="text-xs font-bold text-blue-400 tabular-nums whitespace-nowrap">{relativeDate(item.date_attribution)}</span>
					</div>
				{/each}
			</div>
		</div>
					...
					<div class="px-8 py-5 flex items-center gap-4 hover:bg-slate-50/50 transition-colors">
						...
						<div class="flex-1 min-w-0">
							...
							<p class="text-[10px] font-black text-slate-400 uppercase tracking-tighter mt-0.5">
								{item.motif || 'Mouvement de stock'} · <span class="{variation > 0 ? 'text-emerald-500' : 'text-red-400'}">{relativeDate(item.date_modification)}</span>
							</p>
						</div>
						<span class="font-black text-sm shrink-0 {variation > 0 ? 'text-emerald-600' : 'text-red-500'}">
